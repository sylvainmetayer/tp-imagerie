...
public class TP3_Retailler_Antialiasing implements PlugInFilter {
	...
	//  dialogue  permettant  de fixer la  valeur  du  facteur d’echelle
		...
		for(int y = 0; y < _h; y++ ) {
			for(int x = 0; x < _w; x++) {
				int xx = (int)(x/ratio);
				int yy = (int)(y/ratio);
				// Autre méthode avec getInterpolated
				int iVal = (int) ip.getInterpolatedPixel(xx, yy);
				ipr.putPixel(x, y, iVal);
			}
		}
	...
	public int setup(String arg0, ImagePlus arg1) {
		if (arg0.equals("about")){
			IJ.showMessage("Retailler antialiasing");
